...
export default function CourseQuizPage() {
  ...
  useEffect(() => {
    ...
    const loadQuizData = async () => {
      ...
      try {
        ...
        const fetchedQuizzes = await quizService.getCourseQuizzes(courseData.id, token);
  ...
  const defaultQuestions = [
    {
      title: 'What is the default rendering paradigm of Next.js App Router components?',
      options: ['Client Components', 'React Server Components', 'Static HTML Only', 'Edge Rendered'],
      correct_answer: 1,
    },
    {
      title: 'Which Strapi hook handles user authentication token verification?',
      options: ['Authenticated Role middleware', 'Draft & Publish engine', 'Content-Type Builder', 'Media Library plugin'],
      correct_answer: 0,
    },
  ];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.warn('Quiz submission fallback applied:', err.message);
    } finally {
      setResult({
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="bg-white border border-[var(--color-brand-border)] rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-sm">
        <div>
          <Link href={`/courses/${slug}/learn`} className="text-xs text-[var(--color-brand-primary)] font-semibold hover:underline">
          ...
          <p className="text-xs text-[var(--color-brand-text-muted)] mt-1">
            Test your comprehension to certify course milestones.
          </p>
        </div>

        <span className="px-3.5 py-1.5 rounded-full bg-indigo-50 text-[var(--color-brand-primary)] text-xs font-bold uppercase tracking-wider">
      ...
      {/* Result Card Banner */}
      {result && (
        <div className={`p-6 sm:p-8 rounded-3xl border shadow-sm space-y-3 ${
          ...
          <p className="text-sm text-gray-700">
            {result.passed
              ? 'Great job! You have demonstrated strong understanding of the module core topics.'
              : 'You scored below the 60% passing mark. Review the video modules and re-attempt.'}
          </p>
          <div className="pt-2 flex gap-3">
            ...
            <Link
              ...
              className="px-4 py-2 bg-[var(--color-brand-primary)] text-white rounded-xl text-xs font-semibold hover:bg-[var(--color-brand-primary-hover)] transition"
            >
              Continue Learning
            </Link>
          </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <span className="w-7 h-7 rounded-full bg-indigo-50 text-[var(--color-brand-primary)] font-bold text-xs flex items-center justify-center border border-indigo-100">
                  {qIdx + 1}
                </span>
                  ...
                  return (
                    <button
                      ...
                      className={`text-left p-4 rounded-xl border text-sm font-medium transition flex items-center gap-3 ${
                        isSelected
                          ? 'border-[var(--color-brand-primary)] bg-indigo-50/70 text-[var(--color-brand-primary)]'
                          : 'border-[var(--color-brand-border)] bg-gray-50/40 hover:bg-gray-100 text-gray-700'
                      }`}
                    >
                      <span className={`w-5 h-5 rounded-full border flex items-center justify-center text-xs font-bold ${
                        isSelected ? 'border-[var(--color-brand-primary)] bg-[var(--color-brand-primary)] text-white' : 'border-gray-400 text-transparent'
                      }`}>
                        ✓
                      </span>
                      <span>{opt}</span>
      ...
      {!result && (
        <div className="flex justify-end pt-4">
          <button
            ...
            className="px-8 py-3.5 bg-[var(--color-brand-primary)] hover:bg-[var(--color-brand-primary-hover)] disabled:bg-gray-300 text-white font-semibold rounded-xl text-sm transition shadow-sm"
          >
